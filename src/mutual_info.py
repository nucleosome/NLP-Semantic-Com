...
def mutual_information(joint, marginal, mine_net):
    ...
    mi_lb = torch.mean(t) - torch.log(torch.mean(et))# L: eq9 in DeepSC paper
    return mi_lb, t, et
...
def sample_batch(rec, noise):
    ...
    rec_sample1, rec_sample2 = torch.split(rec, int(rec.shape[0]/2), dim=0)# L: Splits the tensor into chunks. 2022-05-16 divided rec into 2 parts
    noise_sample1, noise_sample2 = torch.split(noise, int(noise.shape[0]/2), dim=0)
    joint = torch.cat((rec_sample1, noise_sample1), 1)# L : joint probabilities 2022-05-16 why this is probability, it just a combination of a transform tx signal and rx signal?
    marg = torch.cat((rec_sample1, noise_sample2), 1)# L : marginal probabilities 2022-05-16
    return joint, marg