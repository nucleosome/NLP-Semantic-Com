...
class Channels():
    ...
    def AWGN(self, Tx_sig, n_var):
        #lll_noise_for_debug = torch.normal(0, n_var, size=Tx_sig.shape).to(device)
        #Rx_sig = Tx_sig + lll_noise_for_debug
...
def train_step(model, src, trg, n_var, pad, opt, criterion, channel, mi_net=None):
    ...
    if mi_net is not None:
        ...
        loss_mine = -mi_lb
        loss = loss + 0.0009 * loss_mine# L:lambda eq14 in DeepSC paper2022-05-16
    # loss = loss_function(pred, trg_real, pad)

    loss.backward()# L: 反向传播计算得到每个参数的梯度值 2022-05-16
    opt.step()# L: 梯度下降执行一步参数更新 2022-05-16
...
def train_mi(model, mi_net, src, n_var, padding_idx, opt, channel):
    """
    L: train with mutul information estimation model

    input-
    model: network structure e.g. DeepSC
    mi_net: mutual information network
    src: sentence?
    n_var: noise var
    padding_idx:
    opt:
    ...
    -output-
    loss_mine.item(): value of the Tensor

    """
    mi_net.train()# L: a notes tell us we start to train
    opt.zero_grad()
    channels = Channels()
    ...
    loss_mine.backward()#L: backward propagation
...
def val_step(model, src, trg, n_var, pad, criterion, channel):
    ...
    loss.item()
    """
    channels = Channels()#L:creat an object in class "Channels"
    trg_inp = trg[:, :-1]#L:all the rows and all the columns except the last column
    trg_real = trg[:, 1:]#L:all the rows and all the columns except the first column

    src_mask, look_ahead_mask = create_masks(src, trg_inp, pad)

    enc_output = model.encoder(src, src_mask)
    channel_enc_output = model.channel_encoder(enc_output)
    Tx_sig = PowerNormalize(channel_enc_output) #L: Normalize power of Tx signal
...
def greedy_decode(model, src, n_var, max_len, padding_idx, start_symbol, channel):# LLL add "Rx_sig" and "Tx_sig" as output 2022-05-17
    """ 
...
def greedy_decode_L(model, src, n_var, max_len, padding_idx, start_symbol, channel):
    """
    output tx signal and rx signal

    LLL add "Rx_sig" and "Tx_sig" as output 2022-05-17
    """
    # create src_mask
    channels = Channels()
    src_mask = (src == padding_idx).unsqueeze(-2).type(torch.FloatTensor).to(device)  # [batch, 1, seq_len]
